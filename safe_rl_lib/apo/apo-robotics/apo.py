...
def apo(env_fn, omega_1, omega_2, k=7.0, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
        ...
        backtrack_iters=100, model_save=False, robotics=None, detailed=True):
    """
    ...
    Args:
        ...
        robotics (str): name of robotics game (None if running continuous game).
        
        detailed (bool): whether to display detailed computation of square item in variance mean

    """
    
    def robotics_env_fn(robotics_name):
        env_name = robotics_name + '-v1'
    ...
    env = robotics_env_fn(robotics)
    obs_dim = env.observation_space['observation'].shape
    act_dim = env.action_space.shape

    # Create actor-critic module
    ac = actor_critic(env.observation_space['observation'], env.action_space, **ac_kwargs).to(device)
    ...
    for epoch in range(epochs):
        for t in range(local_steps_per_epoch):
            ...
            robotics_mode = robotics != None
            timeout = (ep_len == max_ep_len) and (not robotics_mode)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--robotics_name', '-r', type=str,
                        choices=['HandReachDense', 'HandManipulateBlockDense', 'HandManipulateEggDense'])    
    args = parser.parse_args()

    mpi_fork(args.cpu)  # run parallel code with mpi
    
    detailed = 'Detailed' if args.detailed else 'NotDetailed'
    exp_name = args.robotics_name + '_' + args.exp_name + '_' + 'kl' + str(args.target_kl)\
            + '_' + detailed\
    ...
    apo(lambda : create_env(args), actor_critic=core.MLPActorCritic,
        ...
        k=args.k, omega_1=args.omega1, omega_2=args.omega2, robotics=args.robotics_name, detailed=args.detailed)
