...
def apo(env_fn, omega_1, omega_2, k=7.0, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
        steps_per_epoch=4000, epochs=50, gamma=0.99, 
        vf_lr=1e-3, train_v_iters=80, lam=0.97, max_ep_len=1000,
        target_kl=0.01, logger_kwargs=dict(), save_freq=10, backtrack_coeff=0.8, 
        backtrack_iters=100, model_save=False, atari=None, detailed=True):
    """
    ...
    def update():
        ...
        true_1 = val.mean().cpu().detach()
        true_2 = torch.var(val).cpu().detach()

        term_1 = val.mean() + (ratio*disc_adv).mean() - 2*gamma*torch.max(disc_adv)*torch.sqrt(torch.tensor(0.5*abs(approx_kl)).to(device)) / (1-gamma)
        term_1 = term_1.cpu().detach()
        term_2 = tmp_1.mean().cpu().detach()
        term_3 = tmp_2.mean().cpu().detach()
        term_4 = abs(approx_kl)
        term_5 = (L_**2 + 2*L_*val).mean().cpu().detach()
        term_6 = (max(0, term_1))**2
        # 比较[1:]的true_1 - k*true_2 与 
        # [:-1]的term_1 - k*{ mu*abs(tarm_2+term_3*Hmax+Hmax**2)/(1-gamma**2) + 2*gamma**2*mu*sqrt(0.5*term_4)*omega_inf/(1-gamma**2) + mu*term_5 - term_6 + true_2 }
        ...
        logger.store(LossPi=pi_l_old, LossV=v_l_old,
                     ...
                     EpochS = s_ep,
                     True1=true_1, True2=true_2,
                     Term1=term_1, Term2=term_2, Term3=term_3, Term4=term_4, Term5=term_5, Term6=term_6)
    ...
    for epoch in range(epochs):
        ...
        logger.log_tabular('True1', average_only=True)
        logger.log_tabular('True2', average_only=True)
        logger.log_tabular('Term1', average_only=True)
        logger.log_tabular('Term2', average_only=True)
        logger.log_tabular('Term3', average_only=True)
        logger.log_tabular('Term4', average_only=True)
        logger.log_tabular('Term5', average_only=True)
        logger.log_tabular('Term6', average_only=True)
        logger.dump_tabular()       
        
        
def create_env(args):
    env = safe_rl_envs_Engine(configuration(args.task + "_0Hazards"))
    return env
...
if __name__ == '__main__':
    ...
    parser.add_argument('--omega1', type=float)       
    parser.add_argument('--omega2', type=float)       
